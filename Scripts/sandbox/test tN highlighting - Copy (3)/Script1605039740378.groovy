...
//id = 'rtc9'
	...
	if (chapters[i] != lastChapter || verses[i] != lastVerse ) {
		println('Reading spans for ' + chapters[i] + ':' + verses[i])
...
//		            'background-color')
		        if (bgColor == highlighted) {
		            print(('word ' + myNum) + ' is highighted as expected\n')
		        } else {
		            print(('---------- word ' + myNum) + ' is not highlighted but should be\n')
					println('Background color is ' + bgColor)
		        }
		    } else {
		        if (bgColor == highlighted) {
		            print(('++++++++++ word ' + myNum) + ' is highighted but should not be\n')
		        } else {
		            print(('word ' + myNum) + ' is not highlighted as expected\n')
		        }
		    }
		myNum ++
		}
