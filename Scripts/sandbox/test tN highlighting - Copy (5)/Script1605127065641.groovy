...
//		olString = origLang.replace('%',', ')
	...
	if (!ellipsis) {
		...
		if (w > 0) {
			for (span in spans) {
				sp ++
				wordStr = 'word ' + sp + ', ' + span
				s = olString.indexOf(span)
				println('start of span ' + span + ' is ' + s)
				if (s < w) {
					if (bgColors[sp-1] == highlighted) {
					...
					if (bgColors[sp-1] == highlighted) {
						...
						println('---------- ' + msg)
						println('Background color is ' + bgColors[sp-1])
						oFile.append(msg)
					}
				} else {
					if (bgColors[sp-1] == highlighted) {
		...
		println('bypassed because string has ellipsis')
	}
	i++
}